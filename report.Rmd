...
system("cmd.exe", input = "type models\\rasch-hyerarchical.txt")
```
...
check_convergence <- function(jags_out) {
  ...
  K = dim(sims.array)[3]
  check = list()
  check$rhat = numeric(K)
  check$ess  = numeric(K)

  for(i in 1:K) {
    sims = sims.array[,,i]
    check$rhat[i] = sims %>% Rhat
    check$ess[i]  = sims %>% ess_bulk
  }
...
```{r}
dplyr::bind_rows(list(betas=betas.check$rhat%>%as_tibble, rasch=rasch.check$rhat%>%as_tibble), .id = 'source') %>%
...
```{r}
dplyr::bind_rows(list(betas=betas.check$ess%>%as_tibble, rasch=rasch.check$ess%>%as_tibble), .id = 'source') %>%
  ggplot(aes(x=value, fill = source, color=source)) +
  geom_histogram(aes(x=value,y=after_stat(density)),binwidth = 100) + 
  geom_density(aes(x=value,y=after_stat(density)*5), fill=NA) +
...
check_convergence <- function(jags_out) {
  ...
  sims.array %>% apply(c(2,3), .%>% acf(., lag.max = 500, plot=F))
  #return(check)
}

d = check_convergence(jags.betas)
```

```{r}

d%>%map(unlist) %>% lapply(as.data.frame.list) %>% bind_rows()
...
abline(v=T1.y, col='red', lwd = 2)
text(x=220, y=3000, labels = paste('ppp-value:', pppvalue.sum.rasch), col='red')
```
...
freq.theta = freq.theta / mean(freq.tau)
...
# Censored Items: can the model recover data?
What happens if we censor some random ratings? Will the model be able to recover their original value?
