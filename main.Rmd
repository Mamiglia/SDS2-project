...
ggplot(melt(data), aes(Var2,Var1, fill=value)) + 
  geom_raster() +
  theme_void() + theme(aspect.ratio=1) + scale_fill_viridis_c()
```
...
I = nrow(data)
J = ncol(data)
jags_data = list(X = data, I = I, J = J, M = sum(data, na.rm=T))
```
...
s <- Vectorize(function(a,size=46) 
  rep(a,size) %>%
  rdirichlet(1000, .) %>%
  apply(1, max) %>% ifelse(.<M*11/46) %>% mean)

tibble(x=seq(0.1,10, length=100)) %>%
...
jags_output_as_df(jags.dir) %>%
  ...
  mutate(value = pmin(1,value)) %>% # SOME VALUES ARE OVER 1!
